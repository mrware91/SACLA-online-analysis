...
figOpts = { 'dpi':80 , 'xIn':2.9 , 'yIn':2 , 'xLims':xLims , 'yLims':yLims , 'nxTicks':nxTicks , 'nyTicks':nyTicks , 'xLabel':xLabel , 'yLabel':yLabel , 'xUnits':xUnits , 'yUnits':yUnits }

figOptsColor = { 'dpi':80 , 'xIn':2.9 , 'yIn':2 , 'xLims':xLims , 'yLims':yLims , 'zLims':zLims , 'nxTicks':nxTicks , 'nyTicks':nyTicks , 'nzTicks':nzTicks , 'xLabel':xLabel , 'yLabel':yLabel , 'zLabel':zLabel , 'xUnits':xUnits , 'yUnits':yUnits , 'zUnits':zUnits }
...
def grabData(range_type, range_values, beamline, directory, equip):
        try:        
                if range_type == 'events':
                        ...
                        data = Database.loadRunData(beamline, range_values, directory)
...
###############################################################################################
    ...
    def filt_data(self):
        ''' filter data '''
        # also check for nans
        nan_filter_mask = np.isnan( self.plot_x ) | np.isnan( self.plot_y )
    ...
    def plot_scatter(self):
        ...
        x, data = self.filt_x, self.filt_y
        scatterPlot(x, data, **figOpts)
...
###############################################################################################
        ...
        if self.refresh_plot:
                self.replace_with_plot.setGeometry(QtCore.QRect(10, 10, 581, 315))
                plt.clf()
                self.canvas.draw()
